...
function isMatched(D) {
    ...
    //
    //console.log(expects)
    // expects = expects.map((r)=>(ac.parse(r)))
    // expects = expects.map((r)=>(maybeNodeTocode(r)))
    //
    let reals = elel.mapv(attribs,(k,D)=>(D[k]),[node])
    //
    //console.log(reals,funcs)
    //
...
function srchRegExpLiteral(ast,mat,type,condFunc,attribs,funcs,paramsDict) {
    let rslts = []
    for(let i=0;i<mat.length;i++){
        let layer = mat[i]
        for(let j=0;j<layer.length;j++){
...
function nodesToCode(nodes) {
    nodes = nodes.map(escodegen.generate)
    let s = nodes.join('\n')
    return(s)
}
...
//let funcs = [maybeNodeToCode,maybeNodeToCode,maybeNodeToCode]
...
function go(D) {
    ...
    let ast = ac.parse(code, {ranges: true,sourceType:sourceType})
    let mat = esmat.getDescMat(ast)
    //
    let srchFunc = D.srchFunc
    if(srchFunc===undefined) {
        srchFunc = srch
    }
    //
    let rslts = srchFunc(ast,mat,type,isMatched,attribs,funcs)
    let s = rslts.join("\n================================\n")
...
module.exports = {
    go:go,
    maybeNodeToCode:maybeNodeToCode,
    doNothing:doNothing,
    srchRegExpLiteral:srchRegExpLiteral
}

/*
let attribs = ['test','consequent']
let type = "SwitchCase"
let funcs = [maybeNodeToCode,maybeNodeToCode]
